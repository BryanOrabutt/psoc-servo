...
;  Global Symbols
;-----------------------------------------------
export  Motor2_EnableInt
export _Motor2_EnableInt
export  Motor2_DisableInt
export _Motor2_DisableInt
export  Motor2_Start
export _Motor2_Start
export  Motor2_Stop
export _Motor2_Stop
export  Motor2_WritePeriod
export _Motor2_WritePeriod
export  Motor2_WritePulseWidth
export _Motor2_WritePulseWidth
export  Motor2_wReadPulseWidth
export _Motor2_wReadPulseWidth
export  Motor2_wReadCounter
export _Motor2_wReadCounter

; The following functions are deprecated and subject to omission in future releases
;
export  wMotor2_ReadPulseWidth    ; deprecated
export _wMotor2_ReadPulseWidth    ; deprecated
export  wMotor2_ReadCounter       ; deprecated
export _wMotor2_ReadCounter       ; deprecated
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Motor2_EnableInt
;
;  DESCRIPTION:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Motor2_DisableInt
;
;  DESCRIPTION:
...
_Motor2_WritePeriod:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[Motor2_PERIOD_LSB_REG], A
   mov   A, X
   mov   reg[Motor2_PERIOD_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Motor2_WritePulseWidth
;
;  DESCRIPTION:
...
;    functions.
;
 Motor2_WritePulseWidth:
_Motor2_WritePulseWidth:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[Motor2_COMPARE_LSB_REG], A
   mov   A, X
   mov   reg[Motor2_COMPARE_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Motor2_wReadPulseWidth
;
;  DESCRIPTION:
...
;    functions.
;
 Motor2_wReadPulseWidth:
_Motor2_wReadPulseWidth:
 wMotor2_ReadPulseWidth:                         ; this name deprecated
_wMotor2_ReadPulseWidth:                         ; this name deprecated
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   A, reg[Motor2_COMPARE_MSB_REG]
   mov   X, A
   mov   A, reg[Motor2_COMPARE_LSB_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Motor2_wReadCounter
;
;  DESCRIPTION:
...
;
 Motor2_wReadCounter:
_Motor2_wReadCounter:
 wMotor2_ReadCounter:                            ; this name deprecated
_wMotor2_ReadCounter:                            ; this name deprecated
   ...
   mov   A, reg[Motor2_COMPARE_MSB_REG]          ; Save the Compare register on the stack
   push  A                                          ;
   mov   A, reg[Motor2_COMPARE_LSB_REG]          ;
   push  A                                          ;  -stack frame now 2 bytes-
   Motor2_Stop_M                                 ; Disable the PWM function
   M8C_SetBank1                                     ;
   mov   A, reg[Motor2_INPUT_LSB_REG]            ; save the LSB clock input setting
   push  A                                          ;   on the stack (now 3 bytes) and ...
                                                    ;   hold the clock low:
   mov   reg[Motor2_INPUT_LSB_REG], INPUT_REG_NULL
   M8C_SetBank0                                     ; Extract the Count via DR2 register
   mov   A, reg[Motor2_COUNTER_MSB_REG]          ; DR2 <- DR0 (in the MSB block)
   mov   A, reg[Motor2_COMPARE_MSB_REG]          ; Stash the Count MSB on the stack
   push  A                                          ;  -stack frame is now 4 bytes
   mov   A, reg[Motor2_COUNTER_LSB_REG]          ; DR2 <- DR0 (in the LSB block)
   mov   A, reg[Motor2_COMPARE_LSB_REG]          ; Stash the Count LSB on the stack
   ...
   mov   reg[Motor2_INPUT_LSB_REG], A            ;    and restore it
   M8C_SetBank0                                     ;
   Motor2_Start_M                                ; Now re-enable the PWM function
